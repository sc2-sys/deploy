...
""".format(
    ...
    run(
        f"sudo tee {proxy_conf} > /dev/null",
        shell=True,
...
""".format(
    ...
    if "proxies" in daemon_config:
        ...
        for daemon_key, env_key in daemon_proxy_map.items():
            if (
                daemon_key in daemon_config["proxies"]
                and daemon_config["proxies"][daemon_key] != proxy_settings[env_key]
            ):
                raise ValueError(
                    f"Proxy setting {daemon_key} in daemon.json differs from env"
                )
    else:
        # Add new proxy settings
        daemon_config["proxies"] = {
            k: proxy_settings[v] for k, v in daemon_proxy_map.items()
        }

    run(f"sudo mkdir -p {daemon_json_path.parent}", shell=True, check=True)
    run(
        f"sudo tee {daemon_json_path} > /dev/null",
        shell=True,
        input=json.dumps(daemon_config, indent=2).encode(),
        check=True,
    )
    ...
    if "default" in user_config["proxies"]:
        ...
        for config_key, env_key in user_proxy_map.items():
            if (
                config_key in user_config["proxies"]["default"]
                and user_config["proxies"]["default"][config_key]
                != proxy_settings[env_key]
            ):
                raise ValueError(
                    f"Proxy setting {config_key} in config.json differs from env"
                )
    else:
        # Add new proxy settings
        user_config["proxies"]["default"] = {
            k: proxy_settings[v] for k, v in user_proxy_map.items()
        }

    run(f"mkdir -p {user_config_dir}", shell=True, check=True)
    run(
        f"tee {user_config_path} > /dev/null",
        shell=True,
        input=json.dumps(user_config, indent=2).encode(),
        check=True,
    )
